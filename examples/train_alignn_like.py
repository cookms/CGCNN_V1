#!/usr/bin/env python
"""Train the ALIGNN-like model from a CSV file.

Example:
    python examples/train_alignn_like.py --csv data/id_prop.csv --target target --cutoff 5.0
"""
...
from materials_gnn.models import ALIGNNLikeModel
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Cap outgoing j->k bonds per incoming i->j bond in the line graph to reduce ALIGNN memory use",
    )
    parser.add_argument(
        "--max-line-edges",
        type=int,
        default=None,
        help="Hard cap on line-graph angle edges per crystal; useful as a safety valve for very dense graphs",
    )
    parser.add_argument(
    ...
    parser.add_argument("--output-dir", default="runs/alignn_like")
...
def _inference_config_from_args(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "line_graph_kwargs": _line_graph_kwargs_from_args(args),
    }
...
def _experiment_config_from_args(
    ...
    *,
    ...
    return {
        ...
        "script": "examples/train_alignn_like.py",
        "model": {
            "name": "alignn_like",
            "architecture": "ALIGNNLikeModel",
        ...
        "featurization": {
            ...
            "angle_basis_use_cosine": args.angle_basis_use_cosine,
            "learnable_angle_basis": args.learnable_angle_basis,
            "num_rbf": args.num_rbf,
            "num_angle_rbf": args.num_angle_rbf,
        },
        "split": {
...
def main() -> None:
    ...
    train_set, val_set, test_set, split_indices = split_dataset(
        dataset,
        seed=args.seed,
    )
    ...
    model = ALIGNNLikeModel(**_model_config_from_args(args))
