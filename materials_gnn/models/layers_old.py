...
class GatedGraphConv(nn.Module):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor) -> tuple[Tensor, Tensor]:
        ...
        # Under CUDA AMP, MLP outputs may be float16 while embeddings/input node
        # tensors remain float32. ``index_add_`` requires the destination buffer and
        # source tensor to have the same dtype, so create aggregation buffers from
        # the tensors being accumulated rather than from ``x``. This keeps the layer
        # AMP-safe while preserving the normal float32 path when mixed precision is
        # disabled.
        aggregate = weighted_messages.new_zeros((num_nodes, self.node_dim))
        gate_sum = gates.new_zeros((num_nodes, self.node_dim))
        aggregate.index_add_(0, dst, weighted_messages)
        gate_sum.index_add_(0, dst, gates)
